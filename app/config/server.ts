...
export const getServerSideConfig = () => {
  ...
  if (disableGPT4) {
    ...
    customModels += DEFAULT_MODELS.filter(
      (m) =>
        (m.name.startsWith("openai/gpt-4") || m.name.startsWith("openai/chatgpt-4o")) &&
        !m.name.startsWith("openai/gpt-4o-mini"),
    )
      .map((m) => "-" + m.name)
      .join(",");
    if (
      (defaultModel.startsWith("openai/gpt-4") ||
        defaultModel.startsWith("openai/chatgpt-4o")) &&
      !defaultModel.startsWith("openai/gpt-4o-mini")
    )
      defaultModel = "";
    }
  }
